 ...
 * Default: derives the page slug from the URL, fetches existing comments from
 * /api/comments, renders a thread + a post form at the end of <body>, and
 * persists via POST.
 * Per-section threads: add `<div data-comments="my-slug" data-comments-title="Notes"></div>`
 * anywhere in the page — each such element gets its own thread stored under
 * that slug (the slug must be whitelisted in api/comments.py). When any
 * [data-comments] elements exist, the default body-level thread is skipped
 * unless <body data-page-comments> is set.
...
(function () {
  ...
  function injectStyles() {
    ...
    var css =
      ...
      ".rfc-comments.rfc-inline{margin:24px 0 8px;padding:0;max-width:none;}" +
      ...
      ".rfc-comments.rfc-inline h2{font-size:14px;}" +
  ...
  function submit(root, slug, e) {
    ...
    fetch(API, {
      ...
      body: JSON.stringify({ page: slug, author: author, body: body }),
    })
      .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, j: j }; }); })
      .then(function (res) {
  ...
  function buildThread(slug, title, inline) {
    var root = el("section", { class: "rfc-comments" + (inline ? " rfc-inline" : "") });
    root.appendChild(el("h2", { text: title || "Comments" }));
    ...
    form.addEventListener("submit", function (e) { submit(root, slug, e); });
  ...
  function mount() {
    ...
    if (hosts.length) {
      Array.prototype.forEach.call(hosts, function (host) {
        ...
        if (!slug) return;
        host.appendChild(buildThread(slug, host.getAttribute("data-comments-title"), true));
      });
      if (!document.body.hasAttribute("data-page-comments")) return;
    }
    document.body.appendChild(buildThread(PAGE, "Comments", false));
  }
